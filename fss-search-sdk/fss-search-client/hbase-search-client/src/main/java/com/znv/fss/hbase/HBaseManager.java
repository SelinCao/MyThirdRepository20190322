...
import com.znv.fss.hbase.NightFreqSearch.NightFreqSearch;
        ...
        if (type.equals("request") && (sId != null)) {
            switch (sId) {
				...
				case ReadMOB:
                    ...
                    break;
                case NightFreqSearch:
                    search = new NightFreqSearch();
                    break;
                default:
                    log.warn("查询的报表不存在。");
                    break;
        ...
        SearchByImage("以图搜图", "12001"), BlackStaticSearch("黑名单静态比对", "12002"), SearchByTime("以图搜图按时间查询",
            "12003"), RelationshipSearch("历史人物关系查询", "12005"), StayTimeSearch("驻留时间查询", "12006"), SearchByTrial("轨迹查询",
                "12007"), SearchForPeer("同行人轨迹查询",
                    "12008"), WriteMOB("大图写入", "12009"), WriteSmallPicture("小图写入", "12010"), ReadMOB("图片读取", "12011"),NightFreqSearch("频繁夜出", "12012");
        // 成员变量
        private String name;
        private String id;
